		...
		<table class="table">
			...
			<tbody>
				<c:if test="${!empty boardlist}">
					<div>
						<form action="${contextPath}/writeview.do" method="post">
							<input type="hidden" name="num" id="num" value="">
							<c:forEach var="board" items="${boardlist}" varStatus="status">
								<tr>
									<th>${status.count}</th>
									<td>${board.writer}</td>
									<td><button type="submit" style="border:0; outline:0;" onclick="document.getElementById('num').value=${status.count};">${board.title}</button></td>
									<td>${board.content}</td>
									<td style="width: 126px;">${board.date}</td>
								</tr>
							</c:forEach>
						</form>
					</div>
