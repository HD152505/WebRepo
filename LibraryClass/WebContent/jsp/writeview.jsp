...
<body>
	...
	<div class="container">
		<h4>자유게시판</h4>
		<br> <br>
		<table class="table" style="margin-left:100px;width:800px;">
			<thead>
			<%-- <c:set var="item" items="${view}" varStatus="status"> --%>
			
			</thead>
			<tbody>
				<c:forEach var="item" items="${view}" varStatus="status">
				<tr>
					<th colspan="5">자유게시판</th>
				</tr><tr>
					<th>작성자</th><td>${item.writer}</td>
					</tr><tr>
					<th>제목</th><td>${item.title}</td>
					</tr><tr>
					<th>내용</th><td>${item.content}</td>
					</tr><tr>
					<th>날짜</th><td>${item.date}</td>
				</tr>
				<%-- </c:set> --%>
				</c:forEach>
			</tbody>
